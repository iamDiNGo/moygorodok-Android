package com.gorod.moygorodok.ui.announcements.my
...
class MyAnnouncementsViewModel : ViewModel() {
    ...
    var statusFilter: AnnouncementStatus? = null
        private set

    fun setStatus(status: AnnouncementStatus?) {
    ...
    private fun loadPage(page: Int, append: Boolean) {
        ...
        loadJob = viewModelScope.launch {
            val result = repository.fetchMy(status = statusFilter, page = page)
            result.fold(
                onSuccess = { pageData ->
